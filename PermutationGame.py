...
while(res == "y"):
    ...
    print("1. A move denotes a permutation action. Moves must always be cyclic permutations. To implement the cycle (3 1 5) you must enter 315 \n 12 34 is not a valid move as it is 2 different cycles whereas 12 and 34 are induvidually valid")    
    print("")
    print("2. Lists are objects which are affected by moves. Lists are a set of elements enclosed with '[]' \n For example, [1, 2, 4, 5, 3] is a list. If the move 15 is applied, it becomes [3, 2, 4, 5, 1] \n the target object(s) which we are unscrambling is by definition a list")
    print("")
    print("3. There are 2 types of queues. The move queue (denoted by m) and the object queue (denoted by o) \n In the jth sub-iteration of the ith iteration the move m[i] will be applied on the object o[j]")
    print("")
